...
def test_phase_order_and_status() -> None:
    ...
    assert [phase["status"] for phase in phases] == ["complete", "pending", "pending", "pending"]
...
def test_completed_phase_evidence_exists() -> None:
    ...
    assert [phase["id"] for phase in completed] == ["PPC-1"]
    for relative in completed[0]["evidence"]:
